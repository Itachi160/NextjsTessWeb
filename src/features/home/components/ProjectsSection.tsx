...
export default function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    // Debounce ResizeObserver to prevent synchronous reflow spikes when pin-spacer wraps container
  ...
  return (
    <section
      ref={containerRef}
      className="relative z-30 overflow-hidden isolate transform-gpu bg-[#03050d] will-change-transform"
    ...
    >
      ...
      <div className="h-[100dvh] flex items-center w-full">
        <div
          ref={trackRef}
          className="flex items-center gap-5 sm:gap-6 md:gap-8 px-4 sm:px-6 md:pl-12 md:pr-16 overflow-visible w-max will-change-transform transform-gpu"
          style={{ WebkitBackfaceVisibility: 'hidden', backfaceVisibility: 'hidden' }}
        >
          {/* Header Card */}
          <div className="w-[260px] sm:w-[320px] md:w-[440px] shrink-0 flex flex-col justify-center gap-3 sm:gap-4 md:gap-5 pr-2 sm:pr-4 md:pr-8">
          ...
          {projects.map((project) => {
            ...
            return (
              <div
                ...
                className="w-[270px] sm:w-[340px] md:w-[440px] h-[390px] sm:h-[450px] md:h-[520px] shrink-0 glass-card rounded-2xl md:rounded-3xl border border-white/[0.06] relative overflow-hidden flex flex-col group transition-all duration-500 hover:border-cyber-cyan/30 bg-space-darkest/60"
              >
                ...
                <div className="h-[140px] sm:h-[170px] md:h-[200px] relative overflow-hidden rounded-t-2xl md:rounded-t-3xl">
                  <Image
                    src={project.image}
                    alt={project.title}
                    fill
                  ...
                  <div className="absolute top-3.5 left-3.5 sm:top-4 sm:left-4">
                    <span className="text-[9px] sm:text-[10px] uppercase font-mono text-cyber-cyan tracking-widest bg-space-darkest/80 backdrop-blur-sm border border-cyber-cyan/20 px-2.5 py-1 rounded-full font-bold">
                      {project.category}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between flex-grow p-4 sm:p-5 md:p-6 relative z-10">
                  <div>
                    <div className="flex justify-between items-start gap-2">
                      <h4 className="text-base sm:text-lg md:text-xl font-black text-white group-hover:text-cyber-cyan transition-colors leading-tight">
                        {project.title}
                      </h4>
                    ...
                    <p className="text-gray-400 text-[11px] sm:text-xs leading-relaxed mt-2 md:mt-3">
                      {project.desc}
                    </p>
                  </div>

                  <div className="flex flex-col gap-2.5 sm:gap-3 md:gap-4 mt-3">
          ...
          <div className="w-4 sm:w-8 md:w-12 shrink-0" aria-hidden="true" />
        </div>
      </div>
